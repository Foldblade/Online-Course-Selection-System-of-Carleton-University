...
除登录页、关于页外的所有页面均需登录后方可访问。如未登录而访问了除登录页、关于页外的页面，将会跳转到登录页。

在登录页可选是否保持登录状态。若登录失败会出现失败提示，失败超过三次将会在24小时内禁止登录。

![登录页2](ReadMe.assets/image-20200606190227772.png)

### 课程库（首页）

登录成功后将会自动跳转到首页。首页无论是学生或教学秘书都可访问。
